...
class Lock {
  public:
    ...
    bool isHeldByCurrentThread();   // 判断该锁是否被当前线程所拥有

  private:
    char* name;             // 信号量名称，debug用
    Thread* owner;          // 只有拥有锁的线程才能释放锁，因此类中需要有Thread变量来记录当前拥有锁的是哪一个线程
    List* queue;             // 如果锁没人用，那么当一个线程需要请求锁的时候，该线程可以得到锁，否则当锁被占用的时候，申请该锁的线程就会被阻塞，为了记录由于等待该锁而被阻塞的线程，Lock类中需要有一个List的链表存放阻塞的线程。
    bool isBusy;            // 有两种状态：上锁和解锁，因此Lock类中需要有一个bool类型的变量state记录当前锁的状态。当state为true时表示锁已经被占用，否则表示锁是空闲的。
};
